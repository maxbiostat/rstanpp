...
data {
  ...
  vector<lower = 0, upper = 1>[N] m0;    // prior location parameter for m
...
transformed data {
  ...
  vector[N] shape1  = lambda0 * m0;      // prior shape1 param for m
  vector[N] shape2  = lambda0 * (1-m0);  // prior shape2 param for m
}

// The parameters accepted by the model.
parameters {
  vector[K] beta;                     // coefficients for predictors
  vector<lower = 0, upper = 1>[N] m;  // location parameter for conjugate prior (prior response rates)
}
...
model {
  ...
  target += beta_lpdf(m | shape1, shape2);                                                          // log prior density of m
}
