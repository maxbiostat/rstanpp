...
data {
  ...
  real<lower=0>         disp_threshold;    // shape parameter for inverse-gamma prior on precision
  real<lower=0>         disp_probability;    // scale parameter for inverse-gamma prior on precision
...
// p+1 params: p-dim vector of regression coefficients and scalar inverse precision
parameters {
  vector[p] beta;
...
model {
  ...
  real sigma = sqrt(1/precision);
  
  // initial priors
  beta       ~ multi_normal(beta0, Sigma0);        // MVN initial prior on beta
  precision ~ PC(disp_threshold, disp_probability, 0.5);  // inverse-gamma initial prior on precision
