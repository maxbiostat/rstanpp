...
#' @param disp.thresh     shape parameter for inverse-gamma prior on dispersion parameter (for Gaussian and gamma models). Ignored for binomial and Poisson models
#' @param disp.prob      rate parameter for inverse-gamma prior on dispersion parameter (for Gaussian and gamma models). Ignored for binomial and Poisson models
...
#' 
#' @export
logncpp_glm = function(formula, family, histdata, a0, beta0 = NULL, Sigma0 = NULL, offset = NULL, disp.thresh= 2.1, disp.prob = 1.1, method = 'bridge', nsmpl = 1000, bridge.args = NULL, ...) {
  ## check method
  ...
  if ( method == 'bridge' ) {
    for ( i in seq_along(a0) ) {
      lognc[i] = logncpp_glm_bridge(formula, family, histdata, a0[i], beta0, Sigma0, offset, disp.thresh, disp.prob, bridge.args, ...)
    }
  }
  if ( method == 'importance' ){
    for ( i in seq_along(a0) ) {
      lognc[i] = logncpp_glm_importance(formula, family, histdata, a0[i], beta0, Sigma0, offset, disp.thresh, disp.prob, nsmpl = nsmpl)
    }
  }
...
logncpp_glm_bridge = function(formula, family, histdata, a0, beta0, Sigma0, offset, disp.thresh, disp.prob, bridge.args = NULL, ...) {
  fit = glm_npp_prior(
    formula, family, histdata = histdata, a0 = a0, beta0 = beta0, Sigma0 = Sigma0, offset = offset, disp.thresh= disp.thresh, disp.prob = disp.prob, ...
  )
...
#' @export
logncpp_glm_importance = function(formula, family, histdata, a0, beta0, Sigma0, offset, disp.thresh, disp.prob, nsmpl) {
  ## get design matrix
  ...
  if ( family$family %in% c("gaussian", "Gamma") ) {
    standat = c(standat, 'disp_threshold' = disp.thresh, 'disp_probability' = disp.prob)
  }
